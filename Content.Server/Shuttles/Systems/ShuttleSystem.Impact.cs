    ...
    private void ThrowEntitiesOnGrid(EntityUid gridUid, TransformComponent xform, Vector2 direction)
    {
        ...
        foreach (var ent in list)
        {
            ...
            if (direction.LengthSquared() > minsq)
            {
                ...
                _throwing.TryThrow(ent, direction, ent.Comp, Transform(ent), direction.Length(), playSound: false, predicted: false /* Trauma: unpredicted */);
            }
            else
            {
        ...
        for (var i = startIndex; i < endIndex; i++)
        {
            ...
            foreach (var localEnt in entitiesOnTile)
            {
                ...
                else
                {
                    ...
                    _throwing.TryThrow(localEnt, direction, physics, localEnt.Comp, direction.Length(), playSound: false, predicted: false /* Trauma: unpredicted */);
                }
            }
