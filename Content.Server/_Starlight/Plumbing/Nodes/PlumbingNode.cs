...
[DataDefinition]
[Virtual]
public partial class PlumbingNode : Node, IRotatableNode, IPipeNode
{
    /// <summary>
    ...
    public void AddAlwaysReachable(PlumbingNode plumbingNode)
    {
        if (plumbingNode.NodeGroupID != NodeGroupID) return;
        _alwaysReachable ??= new();
        _alwaysReachable.Add(plumbingNode);

        if (NodeGroup != null)
            IoCManager.Resolve<IEntityManager>().System<NodeGroupSystem>().QueueRemakeGroup((BaseNodeGroup)NodeGroup);
    }

    public void RemoveAlwaysReachable(PlumbingNode plumbingNode)
    {
        if (_alwaysReachable == null) return;

        _alwaysReachable.Remove(plumbingNode);
    ...
    /// </summary>
    private IEnumerable<PlumbingNode> LinkableNodesInDirection(
        Vector2i pos,
    ...
    /// </summary>
    protected IEnumerable<PlumbingNode> PipesInDirection(
        Vector2i pos,
        PipeDirection pipeDir,
        Entity<MapGridComponent> grid,
        EntityQuery<NodeContainerComponent> nodeQuery,
